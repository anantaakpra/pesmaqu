...
@section('content')
<div class="hero min-h-[35vh] bg-gray-800 bg-cover bg-center relative" style="background-image: url('https://images.unsplash.com/photo-1509062522246-3755977927d7?q=80&w=2070&auto=format&fit=crop');">
    ...
    <div class="w-full max-w-7xl mx-auto px-4 md:px-8 relative z-10 text-left text-white py-12">
        ...
        <div class="text-sm font-bold mb-3 tracking-wide">
            ...
            <span class="text-[#bf9000]">Berita</span>
        </div>
        
        <h1 class="text-3xl md:text-5xl font-extrabold mb-5 tracking-tight">Berita & Informasi</h1>
...
<div class="bg-gray-50 py-12 px-4 md:px-8 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-10 bg-white p-5 rounded-2xl shadow-sm border border-slate-100 max-w-4xl">
            <form action="/berita" method="GET" class="flex flex-col md:flex-row gap-4 w-full">
                ...
                <div class="flex-grow relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul atau isi berita di sini..." 
                           class="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 focus:border-[#bf9000] focus:ring-2 focus:ring-[#bf9000]/20 transition outline-none bg-gray-50 focus:bg-white text-gray-700 font-medium">
                </div>
                
                <div class="md:w-56 flex-shrink-0">
                    <select name="filter" onchange="this.form.submit()" 
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#bf9000] focus:ring-2 focus:ring-[#bf9000]/20 transition outline-none bg-gray-50 focus:bg-white text-gray-700 font-semibold cursor-pointer">
                ...
                <button type="submit" class="md:hidden bg-[#bf9000] hover:bg-[#a37a00] text-white font-bold py-3 px-6 rounded-xl shadow-md transition">
                    Cari Informasi
                </button>
        ...
        <div class="mb-8 text-sm text-gray-600 font-medium bg-white py-2.5 px-4 rounded-xl border border-gray-200 inline-flex items-center gap-2 shadow-sm">
            <span>Menampilkan hasil untuk: <strong class="text-slate-900">"{{ request('search') }}"</strong></span>
            <a href="/berita" class="text-red-500 hover:underline font-bold ml-2">Clear Filter ×</a>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full mb-12">
            ...
            <div class="col-span-full flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-300 shadow-sm">
                ...
                <p class="text-gray-500 text-sm text-center max-w-sm">Tidak ada berita atau pengumuman yang sesuai dengan kata kunci tersebut. Coba cari dengan topik lain.</p>
                <a href="/berita" class="mt-6 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2.5 px-6 rounded-full transition text-sm">Kembali ke Semua Berita</a>
            </div>
            @endforelse
        </div>
